...
async def tracked_page(request: Request):
    ...
    try:
        ...
        tender_ids = [t.tender_id for t in tracked_tenders if t.tender_id]
        biddings = db.query(BiddingTender).filter(BiddingTender.tender_id.in_(tender_ids)).all() if tender_ids else []
        bidding_map = {b.tender_id: b.to_dict() for b in biddings}

        tracked_list = []
        for t in tracked_tenders:
            td = t.to_dict()
            td["bidding_info"] = bidding_map.get(t.tender_id, None)
            tracked_list.append(td)

        status_changed_count = len([
            t for t in tracked_list if t.get("bidding_info") or t.get("status") != "公開徵求"
        ])

        return templates.TemplateResponse("tracked.html", {
            "request": request,
            "active_page": "tracked",
            "tracked_tenders": tracked_list,
