...
2. **SQLite 鎖定 (Database is locked)**：因為爬蟲與 Web 服務可能同時寫入資料庫，本系統已全域啟用 **PRAGMA journal_mode=WAL**。此模式允許讀寫並行，大幅降低資料庫鎖定機率。若在 Synology NAS 等不支援 WAL 鎖定之檔案系統上，系統將會自動、安全地降級為 **DELETE 模式** 運作。
...
5. **Synology NAS 目錄唯讀權限問題**：NAS 部署常遭遇掛載目錄 `./data` 唯讀權限問題導致容器崩潰。專案已於 `entrypoint.sh` 整合自動診斷：無寫入權限時，會在容器日誌中印出非常詳細的 DSM File Station 權限調整操作步驟引導。
